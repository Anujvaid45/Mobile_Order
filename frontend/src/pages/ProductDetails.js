...
const ProductDetails = () => {
    ...
    const getProduct = async () => {
        try {
            ...
            getSimilarProduct(data?.product._id, data?.product.processor);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const getSimilarProduct = async (pid, cid) => {
        try {
            const { data } = await axios.get(
                `https://mobile-backend-taxn.onrender.com/api/v1/product/related-product/${pid}/${cid}`
            );
        ...
        <Layout title={`${product.name}`}>
            <div className="row container product-details">
                ...
                <div className="col-md-6 product-details-info">
                    ...
                    <h6 className="mb-2">Memory: {product.memory}</h6>
                    <h6 className="mb-2">Processor: {product.processor}</h6>
                    <h6 className="mb-2">Price: <span className="price">Rs{product.price}</span></h6>
                    <button
                    ...
                    {relatedProducts?.map((p) => (
                        ...
                        <div className="card m-2" key={p._id}>
                            <img
                                ...
                                className="card-img-top"
                                alt={p.name}
                                onClick={() => navigate(`/product/${p.slug}`)} style={{ cursor: 'pointer' }}
                            />
