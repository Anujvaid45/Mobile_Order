...
const HomePage = () => {
  ...
  const getTotal = async () => {
    try {
      const { data } = await axios.get(
        "https://mobile-backend-taxn.onrender.com/api/v1/product/product-count"
      );
      setTotal(data?.total);
    } catch (error) {
      console.log(error);
  ...
  const loadMore = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(
        `https://mobile-backend-taxn.onrender.com/api/v1/product/product-list/${page}`
      );
      setLoading(false);
      ...
      setProducts((prevProducts) => {
        if (radio.length || memory) {
          return [
            ...prevProducts,
            ...data?.products.filter((product) => filterProduct(product)),
          ];
        }
  ...
  const filterProduct = async () => {
    try {
      const { data } = await axios.post(
        "https://mobile-backend-taxn.onrender.com/api/v1/product/product-filters",
        {
          radio,
          memory,
        }
      ...
      <div className="container-fluid row mt-3">
        ...
        <div className="col-md-9">
          ...
          <div className="d-flex flex-wrap align-center justify-center">
            {products?.map((p) => (
              <div className="card m-2 padding10" style={{ width: "21rem" }}>
                <img
